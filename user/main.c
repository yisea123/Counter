...
void modbusRTU_msg_task(void *pdata)
{
	...
	while (1){
		if ((u32)OSQPend(modbusRTU_msg, 0, &err) == 0x55){//有MODBUS RTU 请求
			...
			if ((rec_count > (MODBUS_RTU_MIN_LEN - 1)) && cmd_analyze.rec_buf[0] == my_env.modbus_rtu_addr){
				stop_uart1_receive ();
...
{ 
	...
	while (1)
	{
		...
		{        
			...
			if (cmd_analyze.rec_buf[rec_count - 1] == CTRL_C && my_env.tty == TTY_CONSOLE){
				my_env.sys_break = 1;
			}else{
				...
				if (my_env.tty == TTY_CONSOLE){//console
					if (cmd_analyze.rec_buf[rec_count - 1] == 0x0D){
						...
						my_println();
						vTaskCmdAnalyze ();
					}else if ((cmd_analyze.rec_buf[rec_count - 1] == '\b')){
						if (rec_count > 1){//退格键
							uart1_send_data('\b');
							uart1_send_data(' ');
