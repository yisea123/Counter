...
void TIM2_Int_Init(u16 arr,u16 psc)
{
	...
	TIM_TimeBaseStructure.TIM_ClockDivision = ((uint16_t)0x0100); 
...
void TIM3_IRQHandler(void)   
{
	...
	if (TIM_GetITStatus(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0400)), ((uint16_t)0x0001)) != RESET) 
	{
		...
		if (delay ==  (2 * (999 + 1) / 100 )) 
		{
			...
			TIM_SetCompare2(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0400)),led0pwmval);	
			delay = 0;
		}
	}
}
...
{  
	...
	TIM_OCInitStructure.TIM_OCMode = ((uint16_t)0x0070); 
 	TIM_OCInitStructure.TIM_OutputState = ((uint16_t)0x0001); 
	TIM_OCInitStructure.TIM_OCPolarity = ((uint16_t)0x0000); 
	TIM_OC4Init(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), &TIM_OCInitStructure);  

	TIM_OC4PreloadConfig(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), ((uint16_t)0x0008));  
...
void TIM4_IRQHandler(void)   
{
	...
	if (TIM_GetITStatus(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), ((uint16_t)0x0001)) != RESET) 
	{
		...
		if (delay == (2 * (999 + 1) / 100)) 
		{
			...
			if(led0pwmval==100)dir=0;
			if(led0pwmval==0)dir=1;										 
			TIM_SetCompare4(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)),led0pwmval);	
			delay = 0;
		}
	}
}
...
unsigned long long tim5_cur_cnt = 0;
unsigned long long tim5_pre_cnt = 0;
...
unsigned long long get_tim5_ticks (void)
{
	return (tim5_ticks * 65535 + ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0C00))->CNT);
}

void refresh_dma1_cycle (void)
{
	tim5_cur_cnt = tim5_ticks * 65535 + ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0C00))->CNT;
	dma_irq_cycle = tim5_cur_cnt - tim5_pre_cnt;
	tim5_pre_cnt = tim5_cur_cnt;
}
...
{  
	...
	if(TIM_GetITStatus(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x1400)),((uint16_t)0x0001))!=RESET)
	{
		...
		OSQPost(modbusRTU_msg, (void *) 0x55);
	}
	OSIntExit();
}
