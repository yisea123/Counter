...
__align(8) OS_STK DEBUG_TASK_STK[128]; 
...
void * modbusRTUMsgGrp[1];			




OS_STK MODBUSRTU_TASK_STK[128]; 
...
int main (void)
{
	...
	printf(""  "\n");
	printf("" "*****************************************************************" "\n");
	printf("" "*                   Micrium uC/OS-II Start                      *" "\n");
	printf("" "*****************************************************************" "\n");
	printf("" "my_cmd_Start at 0x%08x" "\n",(unsigned int)(&Image$$MY_CMD_RO_SEC$$Base));
	printf("" "my_cmd_End at 0x%08x" "\n",(unsigned int)((unsigned int)(&Image$$MY_CMD_RO_SEC$$Base) + (unsigned int)(&Image$$MY_CMD_RO_SEC$$Length)));
	printf("" "my_cmd_Length 0x%02x" "\n",(unsigned int)(&Image$$MY_CMD_RO_SEC$$Length));
	
	
	printf("" "TIM2 int cycle is %10d us" "\n",(((499 + 1) * (71 + 1)) / 72));
	printf("" "TIM3 int cycle is %10d us" "\n",(((999 + 1) * (71 + 1)) / 72));
	printf("" "TIM4 int cycle is %10d us" "\n",(((999 + 1) * (71 + 1)) / 72));
	printf("" "TIM5 int cycle is %10d us" "\n",(((65535 + 1) * (71 + 1)) / 72));
	printf("" "TIM6 int cycle is %10d us" "\n",(((31250 + 1) * (71 + 1)) / 72));
	printf("" "TIM7 int cycle is %10d us" "\n",(((3999 + 1) * (71 + 1)) / 72));
	
	printf("Nick-Cmd:"  "");
	
	OSInit(); 
	OSTaskCreate(start_task, (void*)0,(OS_STK*)&START_TASK_STK[256-1], 10); 
    OSTaskNameSet(10, (INT8U *)(void *)"start_task", &err);
	OSStart(); 
...
{ 
	...
	cmd_msg=OSQCreate(&cmdMsgGrp[0], 1);	
	
	debug_msg=OSQCreate(&debugMsgGrp[0], 1);	
	
	modbusRTU_msg=OSQCreate(&modbusRTUMsgGrp[0], 1);	
	
	
	OSTaskCreate(cmd_task,(void*)0,(OS_STK*)&CMD_TASK_STK[64-1],7); 
    OSTaskNameSet(7, (INT8U *)(void *)"cmd_task", &err);
	
	OSTaskCreate(led1_task,(void*)0,(OS_STK*)&LED1_TASK_STK[128-1],6); 
    OSTaskNameSet(6, (INT8U *)(void *)"led1_task", &err);
	
	OSTaskCreate(debug_task,(void*)0,(OS_STK*)&DEBUG_TASK_STK[128-1],5);
    OSTaskNameSet(5, (INT8U *)(void *)"debug_task", &err);
	
	OSTaskCreate(modbusRTU_msg_task,(void*)0,(OS_STK*)&MODBUSRTU_TASK_STK[128-1],4);
    OSTaskNameSet(4, (INT8U *)(void *)"modbusRTU_task", &err);
	
	TIM2_Int_Init (499, 71);
	TIM3_PWM_Init (999, 71); 
	TIM4_PWM_Init (999, 71); 
	TIM5_PWM_Init (65535, 71); 
	TIM6_PWM_Init (31250, 71); 
	TIM7_PWM_Init (3999, 71); 
...
void modbusRTU_msg_task(void *pdata)
{
	...
	pdata=pdata; 
	while (1){
		if ((u32)OSQPend(modbusRTU_msg, 0, &err) == 0x55){
			if (rec_count > 1){
				if (cmd_analyze.rec_buf[0] == my_env.modbus_rtu_addr){
					modbus_analyze (cmd_analyze.rec_buf);
					memset (cmd_analyze.rec_buf, 0, rec_count);
					rec_count = 0;
				}
			}
		}
...
{ 
	...
	while (1)
	{
		if ((u32)OSQPend(cmd_msg,0,&err) == 0x55)
		{
			vTaskCmdAnalyze ();
		}
		delay_ms(10); 
...
{ 
	while(1) 
	{ 
		GPIO_ReadOutputDataBit (((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0100)) ? GPIO_WriteBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0100), Bit_RESET) : GPIO_WriteBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0100), Bit_SET);	
		delay_ms(300); 
		GPIO_ReadOutputDataBit (((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0100)) ? GPIO_WriteBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0100), Bit_RESET) : GPIO_WriteBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0100), Bit_SET);	
		delay_ms(300); 
		if (counter_process_cycle > dma_irq_cycle - 15)
		{
			printf("" "\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" "\n");
			printf("" "Error!!! counter_process take too long time" "\n");
			printf("" "counter_process_cycle = %d us" "\n",counter_process_cycle);
			printf("" "dma_irq_cycle - 15 = %d us" "\n",dma_irq_cycle - 15);
			printf("" "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" "\n");
			
		}
		if (counter_process_state != 0 && my_env.tty == TTY_IDLE)
		{
			printf("" "Error!!! counter_process runtime error!!! state:%02d" "\n",counter_process_state);
			printf("Nick-Cmd:"  "");
		}
	}; 
} 
...
{ 
	...
	while (1)
	{
		...
		{
			printf("" "start" "\n");
			start_uart1_dma (g_counter.buf_addr, g_counter.ch[my_env.watch_ch].sample_size * 2);
