...
unsigned long simple_strtoul(const char *cp,char **endp,unsigned int base)
{
	...
	if (*cp == '0') {
		cp++;
		if ((*cp == 'x') && (__isxdigit_helper((* __rt_ctype_table())[cp[1]]) & 128)) {
			base = 16;
			cp++;
		}
		if (!base) {
	...
	while ((__isxdigit_helper((* __rt_ctype_table())[*cp]) & 128) && (value = ((* __rt_ctype_table())[*cp] == 32) ? *cp-'0' : (((* __rt_ctype_table())[*cp] & 8)
...
extern OS_STK START_TASK_STK[256]; 
...
void handle_task (void * pdata)
{
	...
	my_env.tty = TTY_IDLE;		
	state = OSTaskDel(0xFFu);		
	if (state != 0u)			
	{
		printf("" "delete handle_task failed" "\n");
	}	
}
...
{   
	...
	state = OSTaskCreate(handle_task, (void*)(cmd_analyze.rec_buf),
						(OS_STK*)&START_TASK_STK[256-1],
						16); 
	if (state != 0u)
	{
		printf("" "OSTaskCreate failed" "\n");
	}
}
void print_system_env_info (void) 
{                  
	printf("" "----------------------------------------------------" "\n");        
	printf("" "              Compile Time Info                     " "\n");     
	printf("" "              %s %s                    " "\n","Jul 18 2017", "17:16:03");                                                                          
	printf("" "----------------print system env info---------------" "\n");        
	printf("" "sys_break        = %d" "\n",my_env . sys_break);             
	printf("" "watch_ch         = %d" "\n",my_env . watch_ch);                    
	printf("" "roll_count       = %d" "\n",my_env . roll_count);               
	printf("" "print            = %d" "\n",my_env . print);             
	...
	printf("" "uart0_cmd_flag   = %d" "\n",my_env . uart0_cmd_flag);       
...
void print_da_all (void)
{
	...
	while (1)
	{
		...
		for(i=0;i<12;i++)
		{
			...
			printf("" "%02d:%05d  " "",i, After_filter[i]);
			if ((i + 1) % 4 == 0)
				printf(""  "\n");
		}
		printf("" "------------------------------------------" "\n");
...
void print_count (void)
{
	...
	while (1)
	{
		...
		for(i=0;i<12;i++)
		{
			printf("" "%02d:%05d  " "",i, g_counter . ch[i]. cur_count);
			if ((i + 1) % 4 == 0)
				printf(""  "\n");
		}
		printf("" "------------------------------------------" "\n");
...
void print_step (void)
{
	...
	printf("" "------------------------------------------" "\n");
	printf("" "---------live process_step value----------" "\n");
	printf("" "------------------------------------------" "\n");
	my_env.roll_count = 0;
	while (1)
	{
		...
		for(i=0;i<12;i++)
		{
			printf("" "%02d:%05d  " "",i, g_counter . ch[i]. process_step);
			if ((i + 1) % 4 == 0)
				printf(""  "\n");
		}
		printf("" "------------------------------------------" "\n");
...
{                                       
	...
	if (data == 0X03 && my_env.tty == TTY_CONSOLE)
	{
		my_env.sys_break = 1;
		return;
	}	
...
__align(4) cmd_tbl_t __my_cmd_reset __attribute__ ((unused, section("my_cmd_sec"))) = {"reset", 8, 1, do_reset, "reset - reset the system\n", "reset\n"};
...
__align(4) cmd_tbl_t __my_cmd_help __attribute__ ((unused, section("my_cmd_sec"))) = {"help", 16, 1, do_help, "print online help\n", "[command ...]\n" "    - show help information (for 'command')\n" "'help' prints online help for the monitor commands.\n" "Without arguments, it prints a short usage message for all commands.\n" "To get detailed help information for specific commands you can type\n" "'help' with one or more command names as arguments.\n"};
 
cmd_tbl_t __u_boot_cmd_question_mark __attribute__ ((unused, section("my_cmd_sec"))) = {
	"?",	16,	1,	do_help,
	"alias for 'help' \n",
	""
};
...
static int do_help (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	{	 
		...
		int cmd_items = (unsigned int)(&Image$$MY_CMD_RO_SEC$$Length) / sizeof (cmd_tbl_t);	 
		cmd_tbl_t *cmd_array[cmd_items];
		int i, j, swaps;

		 
		cmdtp = (cmd_tbl_t *)(&Image$$MY_CMD_RO_SEC$$Base);
...
int do_task_manager (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	{	
		Rollback ();
		printf("" " counter_process_cycle: %06d us\n" "",counter_process_cycle); 
		printf("" " dma_irq_cycle: %06d us\n" "",dma_irq_cycle);  
		printf("" " detect_chanel_index: %d \n" "",detect_chanel_index);  
		printf("" " chanel_pos_index: %d \n" "",chanel_pos_index);   
		printf("" " #Ticks: %ld  \n" "",OSTime);  
		printf("" " #CtxSw: %ld  \n" "",OSCtxSwCtr);  	
		printf("" " OSIdleCtrRun: %ld  OSIdleCtrMax: %ld  \n" "",OSIdleCtrRun, OSIdleCtrMax);  		
		printf("" " CPU Usage: %02d%%\n" "",OSCPUUsage);  
		printf("" "------------------------------------------" "\n");
		delay_ms (1000);
...
__align(4) cmd_tbl_t __my_cmd_task_manager __attribute__ ((unused, section("my_cmd_sec"))) = {"task_manager", 3, 1, do_task_manager, "task_manager - the os task_manager\n", "task_manager\n"};
...
__align(4) cmd_tbl_t __my_cmd_print __attribute__ ((unused, section("my_cmd_sec"))) = {"print", 3, 1, do_print, "print - print info of the arg\n", "print da 2\nprint da all\n"};
	...
	switch (argc){
		case 3:
			if (strcmp (argv[1], "da") == 0){
				dac1_set_vol (simple_strtoul(argv[2], 0, 10));
				break;
			}else if (strcmp (argv[1], "watch") == 0){
				data_temp = simple_strtoul(argv[2], 0, 10);
				if (data_temp < 12){
					my_env.watch_ch = data_temp;
				}else{
					printf("" "chanel value must be 0 - %d" "\n",12);
				}
				break;
				...
				if (data_temp == 0 || data_temp == 1){
					...
					printf("" "print value must be 0 or 1" "\n");
				}
				break;
			}
			cmd_usage (cmdtp);
...
__align(4) cmd_tbl_t __my_cmd_set __attribute__ ((unused, section("my_cmd_sec"))) = {"set", 4, 1, do_set, "set - set parameter value of the arg\n", "set name value\n"};
...
{
	cmd_tbl_t *cmdtp;
	cmd_tbl_t *cmdtp_temp = (cmd_tbl_t *)(&Image$$MY_CMD_RO_SEC$$Base);	 
	const char *p;
	int len;
	int n_found = 0;
	...
	len = ((p = strchr(cmd, '.')) == 0) ? strlen (cmd) : (p - cmd);

	for (cmdtp = (cmd_tbl_t *)(&Image$$MY_CMD_RO_SEC$$Base);
	     cmdtp != (cmd_tbl_t *)((unsigned int)(&Image$$MY_CMD_RO_SEC$$Base) + (unsigned int)(&Image$$MY_CMD_RO_SEC$$Length));
	     cmdtp++) {
